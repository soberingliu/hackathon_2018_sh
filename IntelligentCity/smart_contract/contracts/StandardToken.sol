...
contract StandardToken is ERC20Protocol {
    ...
    */
    ...
    function approve(address _spender, uint _value) onlyPayloadSize(2 * 32) public returns (bool success) {
        ...
        assert((_value == 0) || (allowed[msg.sender][_spender] == 0));  //在设置允许的钱的时候，首先清0
    ...
    mapping (address => uint) balances;   //地址->余额，即账户有多少钱
    mapping (address => mapping (address => uint)) allowed; //某个地址可以给某个地址多少钱。
}
