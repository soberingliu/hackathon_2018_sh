...
console.log("convertLibFile: ", convertLibFile)
...
/*let eventyiToken = yiTokenContributionInst["EventMint"]({},{fromBlock:1, toBlock:'lastest'});
...
/*let res1 = yiTokenContributionInst.balanceOf(env.AccountAddr[1]);
console.log(res1.toNumber());*/

function GetBalance(address){
    return yiTokenContributionInst.balanceOf(address).toNumber();
}

//transfer

function Transfer(toAddress, amount,callback){
    yiTokenContributionInst.transfer.sendTransaction(toAddress, amount, {from:toAddress, gas:4500000},function(error, result){
        if(error){
            console.log(error);
...
function TransferCross(_fromAddr,_toAddr,amount,callback){
    yiTokenContributionInst.transferCross.sendTransaction(_fromAddr, _toAddr,amount,{from:_fromAddr,gas:4500000},function(err,result){
        if(err){
            //console.log(err);
            callback(err,result);
        }else{
